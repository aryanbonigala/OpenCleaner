...
def explain_item(req: ExplainRequest) -> ExplainResponse:
    ...
    if t == ItemType.process:
        ...
        if is_critical_process(n):
            importance = "Strong heuristic match for OS, security stack, or low-level drivers/anti-cheat — treat as critical."
        ...
        importance = "Runs during logon; affects boot-to-usable time and background resource use."
    elif t == ItemType.scheduled_task:
        importance = "Runs on a timer; may perform updates, maintenance, or vendor housekeeping."
