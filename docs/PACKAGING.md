...
| Windows | **Blocked** | No Windows build environment (VM, physical machine, or CI runner) is available in this session. Wine and cross-compilation are explicitly unsupported for PyInstaller output, so this must be verified on real Windows. See [`docs/WINDOWS_PACKAGED_SIDECAR_AUDIT.md`](WINDOWS_PACKAGED_SIDECAR_AUDIT.md) for the detailed gap audit (`.exe` naming, resource config, script portability, signal cleanup). |
